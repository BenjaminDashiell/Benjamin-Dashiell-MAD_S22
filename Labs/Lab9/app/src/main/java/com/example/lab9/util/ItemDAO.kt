...
open class ItemDAO() {
    ...
    fun addItem(item:Item){ realmDB.executeTransactionAsync{transactionRealm ->
        transactionRealm.insert(item)
    }}

    fun deleteItem(id:String){ realmDB.executeTransactionAsync{transactionRealm ->
        transactionRealm.where(Item::class.java).equalTo("id", id).findAll().deleteAllFromRealm()
    }}

    fun deleteAll(){ realmDB.executeTransactionAsync{transactionRealm ->
        transactionRealm.deleteAll()
    }}

    fun close(){ realmDB.close()
    }
}